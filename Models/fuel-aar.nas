...
#		contact	with us.
	...
	if (enabled){# check that AI Models are enabled, otherwise don't bother
		foreach(a; AllAircraft) {
			callsign_node = a.getNode("callsign");
			
			print ("tanker callsign" , callsign_node.getValue());
			
			append(allTankers, a);
		}

		foreach(m; AllMultiplayer) {
			callsign_node = m.getNode("callsign");
			tanker_node = m.getNode("tanker");
			tanker = tanker_node.getValue();

			print ("tanker callsign ", callsign_node.getValue(), " tanker ", tanker );
		...
		print ("tankers ", size(allTankers));

		if ( size(allTankers) >= 1 ) {
			foreach(t; allTankers) {
				contact_node = t.getNode("refuel/contact");
				contact = contact_node.getValue();
				callsign_node = t.getNode("callsign");
				print (" tanker contact ", contact ,
						" tanker callsign ", callsign_node.getValue());
